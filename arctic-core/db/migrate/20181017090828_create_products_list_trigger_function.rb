class CreateProductsListTriggerFunction < ActiveRecord::Migration[5.2]
  def up
    execute <<-SQL
      CREATE OR REPLACE FUNCTION shadow_products_updated_function()
      RETURNS TRIGGER AS $$
        BEGIN
  ...
  def down
    execute 'DROP FUNCTION shadow_products_updated_function'
  end
end
