...
RSpec.describe Amazon::Workers::Images, type: :worker do
  ...
  it "performs correctly" do
    ...
    expect(Amazon::Workers::ProductState).to receive(:perform_async).with shop_id, 'abcdef123', :completed
    ...
    expect(core_api).to receive(:synced).with(shop_id, :products).exactly(1).times
