...
RSpec.describe Amazon::Workers::Inventory, type: :worker do
  ...
  before do
    ...
    allow(core_api).to receive(:list_products).with(shop_id, all: nil).and_return products
    ...
    expect(instance).to receive(:archive_xml).with('submitted')
    ...
    expect(instance).to receive(:archive_xml).with('response', feed_result_xml)
  ...
  context 'with continue option' do
    before do
      ...
      expect(Amazon::Workers::Prices).to receive(:perform_async).with(shop_id, options)
    end
  end

  context 'ignoring product errors' do
    before do
      options[:ignore_errors] = true
      expect(core_api).to receive(:list_products).with(shop_id, { all: true }).and_return products
      expect(core_api).to receive(:synced).with(shop_id, :products)
    end
